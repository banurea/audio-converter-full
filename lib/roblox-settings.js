...
function loadRobloxSettings({ rootDir = process.cwd(), env = process.env, fsModule = fs } = {}) {
  ...
  const merged = normalizeSettings({
    apiKey: env.ROBLOX_API_KEY ?? stored.apiKey ?? '',
    creatorId: env.ROBLOX_CREATOR_ID ?? stored.creatorId ?? '',
    creatorType: env.ROBLOX_CREATOR_TYPE ?? stored.creatorType ?? 'user',
    uploadUrl: env.ROBLOX_UPLOAD_URL ?? stored.uploadUrl ?? 'https://apis.roblox.com/assets/v1/assets',
    setPermissionUrl: env.ROBLOX_SET_PERMISSION_URL ?? stored.setPermissionUrl ?? ''
  });
