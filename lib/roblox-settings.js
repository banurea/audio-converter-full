...
function resolveRobloxSettings(settings = {}, { rootDir = process.cwd(), env = process.env, fsModule = fs } = {}) {
  ...
  return normalizeSettings({
    apiKey: settings.apiKey !== undefined ? settings.apiKey : env.ROBLOX_API_KEY !== undefined ? env.ROBLOX_API_KEY : stored.apiKey ?? '',
    creatorId: settings.creatorId !== undefined ? settings.creatorId : env.ROBLOX_CREATOR_ID !== undefined ? env.ROBLOX_CREATOR_ID : stored.creatorId ?? '',
    creatorType: settings.creatorType !== undefined ? settings.creatorType : env.ROBLOX_CREATOR_TYPE !== undefined ? env.ROBLOX_CREATOR_TYPE : stored.creatorType ?? 'user',
    uploadUrl: settings.uploadUrl !== undefined ? settings.uploadUrl : env.ROBLOX_UPLOAD_URL !== undefined ? env.ROBLOX_UPLOAD_URL : stored.uploadUrl ?? 'https://apis.roblox.com/assets/v1/assets',
    setPermissionUrl: settings.setPermissionUrl !== undefined ? settings.setPermissionUrl : env.ROBLOX_SET_PERMISSION_URL !== undefined ? env.ROBLOX_SET_PERMISSION_URL : stored.setPermissionUrl ?? ''
  });
}
