...
function getFriendlyYoutubeError(message = "") {
  if (isYoutubeBotBlockedError(message)) {
    ...
    const cookieHint = lower.includes("cookies")
      ? " Pastikan YT_DLP_COOKIES_CONTENT / YT_DLP_COOKIES_BASE64 / YT_DLP_COOKIES_URL berisi cookies YouTube terbaru."
      : "";

    return `YouTube memblokir ekstraksi dari server ini. Coba upload file audio langsung, gunakan link lain, atau perbarui cookie YouTube kamu.${cookieHint}`;
  }
