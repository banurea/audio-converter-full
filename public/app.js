...
function getLocalRobloxSettings() {
  if (
    !robloxApiKeyInput ||
    !robloxCreatorIdInput ||
    !robloxCreatorTypeSelect ||
    !robloxUploadUrlInput ||
    !robloxPermissionUrlInput
  ) {
    return {
      apiKey: "",
      creatorId: "",
  ...
  return {
    ...
    uploadUrl:
      String(robloxUploadUrlInput.value || "").trim() ||
...
function loadRobloxSettingsFromLocalStorage() {
  ...
  robloxUploadUrlInput.value =
    stored.uploadUrl || "https://apis.roblox.com/assets/v1/assets";
...
function saveRobloxSettingsToLocalStorage() {
  ...
  try {
    ...
    window.localStorage.setItem(
      LOCAL_STORAGE_ROBLOX_KEY,
      JSON.stringify(settings),
    );
    robloxSettingsStatus.textContent =
      "Tersimpan. Bisa diubah lagi kapan saja.";
...
function showAlert(message, ok = false) {
  ...
  alertBox.style.borderColor = ok
    ? "rgba(0,209,117,.35)"
    : "rgba(255,45,53,.32)";
  alertBox.style.background = ok
    ? "rgba(0,209,117,.12)"
    : "rgba(255,45,53,.12)";
...
function getSuggestedTitle() {
  ...
  if (url) {
    ...
    const currentTitle = currentMeta?.title && String(currentMeta.title).trim();
    if (currentTitle && currentTitle.toLowerCase() !== "audio") {
      return currentTitle;
    }
    if (isYoutubeUrl(url)) {
      return "";
    }
    return fallbackUrlTitle;
  }

  const currentTitle = currentMeta?.title && String(currentMeta.title).trim();
  return currentTitle && currentTitle.toLowerCase() !== "audio"
    ? currentTitle
    : "audio";
}
...
function createLinkRow(value = "") {
  ...
  const queueMetadata = () => {
    ...
    batchLinkStates[row.dataset.linkId] = {
      loading: true,
      title: "",
      duration: null,
      provider: "",
  ...
  if (hasLinks) {
    ...
    rows.forEach((row, idx) => {
      ...
      if (state.loading) {
        ...
        const durationLabel = state.duration
          ? ` • ${formatMinutes(state.duration)} menit`
          : "";
...
function renderBatchResult(results) {
  ...
  results.forEach((item, idx) => {
    ...
    const duration = item.durationSec
      ? ` • ${formatMinutes(item.durationSec)} menit`
      : "";
    ...
    status.textContent =
      item.ok === false
        ? `ERROR: ${item.error || "Gagal"}`
        : `SUKSES: ${item.fileName || item.url || "output tersedia"}`;
    status.className =
      item.ok === false ? "batchResultError" : "batchResultSuccess";
...
function selectFiles(files) {
  ...
  urlInput.value = "";
  linkList
    .querySelectorAll(".batchLinkInput")
...
$("clearBtn").addEventListener("click", () => {
  ...
  note.textContent =
    "Paste link YouTube/direct audio atau upload file, lalu convert ke .ogg.";
  hideAlert();
  resetUploadStatus();
...
function showTab(name) {
  ...
  allSections.forEach((id) => {
    ...
    if (shouldShow) el.classList.remove("tabContentHidden");
    else el.classList.add("tabContentHidden");
  });
...
async function loadMetadata(force = false) {
  ...
  if (
    ...
    (metadataLoadingUrl === url ||
      (currentMeta?.sourceUrl === url && currentMeta?.loaded))
  )
    return;
  ...
  try {
    ...
    const resolvedTitle =
      ...
      String(data.title).toLowerCase() !== "audio"
        ? String(data.title).trim()
        : fallbackTitle;
    ...
    note.textContent =
      ...
      (currentMeta.duration
        ? `Duration: ${currentMeta.duration}s`
        : "Siap dikonversi ke OGG.");
    ...
    currentMeta = {
      ...
      title:
        ...
        String(currentMeta.title).toLowerCase() !== "audio"
          ? String(currentMeta.title).trim()
          : fallbackTitle,
    };
    title.textContent = currentMeta.title || "Audio";
    ...
    note.textContent =
      "Metadata gagal dibaca, tapi kamu masih bisa coba convert.";
    showAlert(err.message);
  } finally {
...
convertBtn.addEventListener("click", async () => {
  ...
  if (url && batchLinks.length) {
    return showAlert(
      "Pilih URL tunggal di atas, atau gunakan daftar link di bawah, bukan keduanya.",
    );
  }
  ...
  form.append(
    "pitchMode",
    document.querySelector('input[name="pitchMode"]:checked')?.value ||
      "chipmunk",
  );
  ...
  if (selectedFiles.length) {
    ...
    form.append("url", url);
    form.append("robloxSettings", JSON.stringify(getLocalRobloxSettings()));
    const titleValue = getSuggestedTitle();
    if (titleValue) {
  ...
  try {
    ...
    if (data.results && data.results.length) {
      ...
      $("jobId").textContent =
        `BATCH ${data.count || data.results.length} items`;
      $("resultTitle").textContent =
        `${data.count || data.results.length} item dikonversi`;
      renderBatchResult(data.results);
      showAlert(
        `BATCH KONVERSI ${data.count || data.results.length} ITEM SELESAI`,
        true,
      );
      batchFiles = data.results.flatMap((result) => {
        if (result.fileList && result.fileList.length) {
          return result.fileList.map((p) => ({
            ...p,
      ...
      batchFiles = firstResult.fileList || [
        {
          fileName: firstResult.fileName,
          url: firstResult.url,
          size: firstResult.size || 0,
          taskTitle: firstResult.title || "",
        },
      ];
    }
    ...
    const fileUrl =
      firstResult.url ||
      (firstResult.fileList &&
        firstResult.fileList[0] &&
        firstResult.fileList[0].url);
    const fileName =
      firstResult.fileName ||
      (firstResult.fileList &&
        firstResult.fileList[0] &&
        firstResult.fileList[0].fileName);
    ...
    $("finalSpeed").textContent = (
      firstResult.robloxPlaybackSpeed ||
      data.robloxPlaybackSpeed ||
      calcRobloxPlaybackSpeed()
    ).toFixed(4);
...
if (saveRobloxSettingsBtn) {
  saveRobloxSettingsBtn.addEventListener(
    "click",
    saveRobloxSettingsToLocalStorage,
  );
}
...
$("uploadRobloxBtn").addEventListener("click", async () => {
  ...
  try {
    if (isDirectOggUpload) {
      ...
      (!lastConvertedFiles || lastConvertedFiles.length <= 1)
    ) {
      const payload = {
      ...
      uploadStatus.textContent =
        "Terdapat beberapa part. Gunakan tombol Upload All.";
      uploadRobloxBtn.disabled = false;
      uploadRobloxBtn.textContent = "⇧ UPLOAD";
      return;
    } else {
      return showAlert(
        "Kirim file .ogg langsung, atau convert audio dulu untuk mendapatkan fileName.",
      );
    }
...
function renderPartsList(parts) {
  ...
  if (uploadAllBtn)
    uploadAllBtn.style.display = parts.length > 1 ? "inline-block" : "none";
  parts.forEach((p, idx) => {
    const row = document.createElement("div");
...
$("uploadAllBtn").addEventListener("click", async () => {
  if (!lastConvertedFiles || !lastConvertedFiles.length)
    return showAlert("Tidak ada part untuk diupload.");
  ...
  try {
    ...
    if (shareWithIds && shareWithIds.length)
      payload.shareWithIds = shareWithIds;
